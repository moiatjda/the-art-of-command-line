...
La fluidesa en la linia de comandes és una habilitat normalment descuidada i considerada arcana, però millora la teva flexibilitat i productivitat com a enginyer tant de manera obvia com a sutil. 
Aquesta és una selecció de notes i consells per fer servir la linia de comandes que hem trobat útils a l'hora de treballar amb Linux. Alguns consells són bàsics, i d'altres són molt específics, sofisticats o obscurs. Aquesta pàgina no és llarga, pero si pots fer servir i recordar els continguts d'aquí, saps un munt. 

Aquest treball és el resultat de molts [many authors and translators](AUTHORS.md).
Alguns d'aquests
