...
*Nota: Estic planejant revisar això i buscant un nou coautor per ajudar a ampliar-ho a una guia més exhaustiva. Tot i que és molt popular, podria ser més ampli i una mica més profund. Si us agrada escriure i esteu a punt de ser un expert en aquest material i esteu disposats a considerar ajudar, si us plau deixeu-me una nota a josh (0x40) holloway.com. –[jlevy](https://github.com/jlevy), [Holloway](https://www.holloway.com). Gràcies!*
...
- [Comandes d'una línia](#una-línia)
...
La fluïdesa en el terminal és una destresa sovint abandonada i considerada arcaica, però aquesta millora la teva flexibilitat i productivitat com a enginyer de formes òbvies i subtils. Aquesta és una selecció de notes i consells per a usar el terminal que trobem útils en treballar en Linux. Alguns consells són elementals i alguns bastant específics, sofisticats o foscos. Aquesta pàgina no és llarga, però si pots usar i recordar tots els punts aquí mostrats, sabràs un munt.
...
però s'ha mogut des de llavors a Github, on persones més talentoses que l'autor han fet nombroses millores.
[**Envia una pregunta**](https://airtable.com/shrzMhx00YiIVAWJg) si teniu una pregunta relacionada amb la línia d'ordres. [**Si us plau contribueix**](/Contributing.md) Si veus un error o alguna cosa que podria ser millor!

## Meta

Abast:

- Aquesta guia és tant per a principiants com per a experimentats. Els objectius són *diversitat* (tot importa), *especificitat* (exemples concrets del cas més comú), i *concisió* (evitar coses que no són essencials o insignificants que puguis buscar fàcilment en un altre lloc). Cada consell és essencial en alguna situació o significativament pot estalviar temps comparat amb altres alternatives.
- Està escrita per a Linux, amb excepció de la seccions "[Només per a US X](#us-x-only)" i "[Només per a Windows](#només-windows)". Molts dels altres punts apliquen o poden ser instal·lats en altres Unices, OS X o fins i tot Cygwin.
- Es centra en Bash interactiu, encara que molts dels consells apliquen per a uns altres shells i al Bash scripting en general.
- Inclou tant comandes "estàndard" Unix així com els que requereixen instal·lacions de paquets especials -- sempre que siguin prou importants per a merèixer la seva inclusió.

Notes:
- Per mantenir-ho en una pàgina, el contingut s'inclou implícitament per referència. Ets prou intel·ligent per buscar més detalls en un altre lloc un cop coneixes la idea o l'ordre a Google. Utilitza `apt`, `yum`, `dnf`, `pacman`, `pip` o `brew` (com correspongui) per a instal·lar programes nous.
- Utilitza [Explainshell](http://explainshell.com/) per obtenir un desglossament útil del que fan les ordres, les opcions, les 'canonades' (pipes), etc.
...
### Vies per obtenir eines Unix a Windows
...
### Eines útils per a la línia de comandes de Windows
